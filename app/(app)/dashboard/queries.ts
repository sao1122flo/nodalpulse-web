...
export const MARKET_TO_JURISDICTIONS: Record<string, string[]> = {
  ...
  // NJ-BPU / MD-PSC / VA-SCC (NJ + MD + VA state PUCs) are geographically PJM — a PJM subscriber sees them.
  PJM:   ["PJM-FERC", "PJM", "NJ-BPU", "MD-PSC", "VA-SCC", "FERC"],
  FERC:  ["FERC"],
}
...
export async function getDiscoveryHits(
  ...
  const sinceStr = new Date(Date.now() - sinceDays * 86_400_000).toISOString().slice(0, 10)
